...
class AsymmetricConvBlock(nn.Module):
    ...
    def forward(self, feature):
        square_feature = self.square_block(feature)
        horizontal_feature = self.horizontal_block(feature)
...
class Backbone(nn.Module):
    def __init__(self, in_channels, out_channels, layer_nums, layer_strides):
        ...
        for i in range(len(layer_strides)):
            blocks = []
            blocks.append(AsymmetricConvBlock(in_channels, out_channels[i], stride=layer_strides[i], padding=1))
            for _ in range(layer_nums[i]):
                blocks.append(AsymmetricConvBlock(out_channels[i], out_channels[i], stride=1, padding=1))

            in_channels = out_channels[i]
            self.multi_blocks.append(nn.Sequential(*blocks))

        # init kaiming như Conv2d chuẩn
